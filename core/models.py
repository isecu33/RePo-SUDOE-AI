...
class ConversationContext(models.Model):
    ...
    chat_session = models.OneToOneField(
        ChatSession, on_delete=models.CASCADE, related_name="molecular_context"
    )
    current_intent = models.CharField(
        max_length=20, choices=INTENT_CHOICES, default="general"
    )
    extracted_parameters = models.JSONField(
        default=dict, help_text="Stores extracted drug, gene, and structure parameters"
    )
    ...
    awaiting_confirmation = models.BooleanField(
        default=False, help_text="Whether the system is waiting for user confirmation"
    )
    available_structures = models.JSONField(
        default=list, help_text="List of available protein structures for selection"
    )
...
class StructureCache(models.Model):
    ...
    pdb_code = models.CharField(
        max_length=10, unique=True, db_index=True, verbose_name="PDB Code"
    )
    structure_data = models.TextField(
        verbose_name="PDB Structure Data", help_text="Raw PDB file content"
    )
    download_url = models.URLField(
        verbose_name="Download URL", help_text="Original download URL"
    )
    file_size = models.PositiveIntegerField(verbose_name="File Size (bytes)")

    # Validation flags
    is_valid_structure = models.BooleanField(
        default=True, help_text="Whether RDKit could successfully parse this structure"
    )
    validation_error = models.TextField(
        blank=True, help_text="Error message if structure validation failed"
    )
...
class DockingJob(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="docking_jobs"
    )
    chat_session = models.ForeignKey(
        ChatSession,
        on_delete=models.CASCADE,
    ...
    structure = models.CharField(
        max_length=20, help_text="Código PDB de la estructura seleccionada"
    )
    receptor_path = models.CharField(
        max_length=1024, help_text="Ruta absoluta al archivo PDB del receptor"
    )
    drug_path = models.CharField(
        max_length=1024, help_text="Ruta absoluta al archivo SDF del fármaco"
    )
    vina_config = models.JSONField(
        default=dict,
        blank=True,
        help_text="Configuración personalizada de AutoDock Vina",
    )
    experiment_analysis = models.JSONField(
        default=dict,
        blank=True,
    ...
    def mark_failed(self, error_message: str, result_data: dict = None):
        ...
        self.finished_at = timezone.now()
        self.save(
            update_fields=["status", "error_message", "result_data", "finished_at"]
        )
