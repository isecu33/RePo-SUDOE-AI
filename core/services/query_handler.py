...
class QueryHandler:
    ...
    def __init__(self, language="es", user=None):
        ...
        try:
            ...
            print(
                f"[WARN] Proveedor de IA del usuario no disponible ({e}); usando OpenAI por defecto"
            )
        ...
        self.genes_db_path = os.path.join(
            os.path.dirname(__file__), "../data/genes/genes_db.xlsx"
        )
        self.drugs_db_path = os.path.join(
            os.path.dirname(__file__), "../data/drugs/drug_db.xlsx"
        )
        ...
        print(f"[DEBUG] QueryHandler inicializado:")  # noqa: F541
        print(f"  - Directorio del archivo: {os.path.dirname(__file__)}")
        print(
            f"  - Temp input dir (core/input): {os.path.abspath(self.temp_input_dir)}"
        )
    ...
    def process_query(
        ...
        try:
            ...
            if intent == "docking":
                return self.handle_docking_flow(
                    user_query,
            ...
            if (
                ...
                return {
                    "type": "api_error",
                    "error": _(
                        "El servicio de IA no está disponible temporalmente. Por favor, contacta al administrador."
                    ),
                    "user_message": _(
                        "Lo siento, hay un problema de configuración en el sistema. Por favor, inténtalo más tarde o contacta con el soporte técnico."  # noqa: E501
                    ),
            ...
            if "api key" in error_str.lower() or "authentication" in error_str.lower():
                return {
                    ...
                    "user_message": _(
                        "Lo siento, hay un problema de configuración. Por favor, contacta al administrador."
                    ),
                    "technical_details": error_str,
                }
            ...
            if "rate limit" in error_str.lower() or "quota" in error_str.lower():
                return {
                    "type": "api_error",
                    "error": _(
                        "Se ha excedido el límite de consultas. Intenta de nuevo en unos minutos."
                    ),
                    "user_message": _(
                        "El sistema está recibiendo muchas consultas. Por favor, espera un momento e inténtalo de nuevo."  # noqa: E501
                    ),
                    "technical_details": error_str,
                }
            ...
            if "timeout" in error_str.lower():
                return {
                    ...
                    "user_message": _(
                        "La consulta está tardando más de lo normal. Por favor, inténtalo de nuevo."
                    ),
                    "technical_details": error_str,
                }
            ...
            return {
                ...
                "error": _(
                    "Error procesando tu consulta. Por favor, inténtalo de nuevo."
                ),
                "user_message": _(
                    "Lo siento, ha ocurrido un error al procesar tu consulta. Por favor, inténtalo de nuevo o reformula tu pregunta."  # noqa: E501
                ),
                "technical_details": error_str,
            }
    ...
    def classify_intent(self, user_query):
        ...
        if self.language == "en":
            ...
            - "docking": Perform a molecular docking experiment (use words like "experiment", "docking", "simulate", "analyze interaction")  # noqa: E501
            ...
            - "docking": Realizar un experimento de docking molecular (usar palabras como "experimento", "docking", "simular", "analizar interacción")  # noqa: E501
        ...
        content = self.ai_provider.generate_response(
            system_prompt, user_query, temperature=0
        )
    ...
    def handle_information_query(self, user_query):
        ...
        try:
            ...
            context = self.prepare_combined_database_context(
                genes_df, drugs_df, user_query
            )

            if self.language == "en":
                system_prompt = f"""
                You are an expert in molecular biology. You have access to a database with information about genes, proteins, and drugs.  # noqa: E501

                Database information:
                {context}

                Answer the user's query in a natural and educational way, using the provided data.
                If the question is about general statistics (like total number of proteins), calculate the answer based on the data.  # noqa: E501

                IMPORTANT: You MUST respond ONLY in ENGLISH. Do not use Spanish or any other language.
                """
            else:
                system_prompt = f"""
                Eres un experto en biología molecular. Tienes acceso a una base de datos con información sobre genes, proteínas y medicamentos.  # noqa: E501

                Información de la base de datos:
                {context}

                Responde la consulta del usuario de manera natural y educativa, usando los datos proporcionados.
                Si la pregunta es sobre estadísticas generales (como número total de proteínas), calcula la respuesta basándote en los datos.  # noqa: E501

                IMPORTANTE: Debes responder SOLO en ESPAÑOL. No uses inglés ni ningún otro idioma.
                """

            content = self.ai_provider.generate_response(
                system_prompt, user_query, temperature=0.7
            )

            return {"type": "information", "response": content}

        except Exception as e:
            return {"error": f"Error procesando consulta informativa: {str(e)}"}

    def handle_docking_flow(
        self,
        user_query,
        estructura_seleccionada=None,
        confirmacion=False,
        ...
        if not drug or not gene:
            return {
                "error": "No se pudo identificar el medicamento y/o el gen en tu consulta"
            }
        ...
        if not confirmacion:
            return {
                "type": "confirmation",
                "message": _(
                    "¿Quieres lanzar un experimento de docking molecular entre %(drug)s y %(gene)s?"
                )
                % {"drug": drug, "gene": gene},
                "drug": drug,
                "gene": gene,
        ...
        if not estructura_seleccionada:
            if len(estructuras) == 1:
                ...
                return {
                    ...
                    "message": _("Se encontraron múltiples estructuras para %(gene)s:")
                    % {"gene": gene},
                    "options": estructuras,
                    "drug": drug,
                    "gene": gene,
        ...
        selected_structure = next(
            (s for s in estructuras if s["id"] == estructura_seleccionada), None
        )
        ...
        try:
            ...
            if not pdb_path:
                return {
                    ...
                    "user_message": _(
                        "No se pudo descargar la estructura 3D de la proteína. Por favor, inténtalo de nuevo más tarde."
                    ),
                }
            if not sdf_path:
                return {
                    ...
                    "user_message": _(
                        "No se pudo procesar la estructura del medicamento. Por favor, inténtalo de nuevo."
                    ),
                }
            ...
            return {
                ...
                "user_message": _(
                    "Ocurrió un error al preparar los archivos necesarios para el docking. Por favor, inténtalo de nuevo."  # noqa: E501
                ),
            }

        # Paso 5.5: Generar análisis predictivo del experimento
        print(
            f"[Análisis] Generando análisis predictivo del experimento {drug} - {gene}..."
        )
        try:
            predictive_analysis = self.generate_experiment_analysis(
                drug, gene, selected_structure["id"]
            )
            print(f"[Análisis] Análisis generado exitosamente")  # noqa: F541
        ...
        if manual_mode:
            return {
                ...
                "files": {"gene": pdb_path, "drug": sdf_path},
                "instructions": _(
                    "Descarga los archivos y configura AutoDock Vina manualmente"
                ),
    ...
    def extract_docking_params(self, user_query):
        ...
        content = self.ai_provider.generate_response(
            system_prompt, user_query, temperature=0.3
        ).strip()
        json_match = re.search(r"\{[\s\S]*\}", content)
        if not json_match:
            raise ValueError(
                f"La respuesta de OpenAI no contiene JSON válido: {content}"
            )
    ...
    def download_pdb_file(self, pdb_id):
        ...
        if os.path.exists(final_file_path):
            print(
                f"[PDB] Archivo procesado {pdb_id}.pdb ya existe en {final_file_path}"
            )
            ...
            if os.path.getsize(final_file_path) > 0:
                ...
                print(
                    f"[PDB] Archivo existe pero está vacío, re-procesando..."  # noqa: F541
                )  # noqa: F541
        ...
        try:
            ...
            print(
                f"[PDB] Descarga completada: {original_size} bytes en {temp_file_path}"
            )
            ...
            print(
                f"[PDB] Usando archivo PDB sin preprocesamiento externo"  # noqa: F541
            )  # noqa: F541
            print(
                f"[PDB] Vina manejará el preprocesamiento internamente con pdb4amber"  # noqa: F541
            )  # noqa: F541

            # PASO 2: Mover archivo directamente al directorio final
            print(f"[PDB] --- MOVIENDO A DIRECTORIO FINAL ---")  # noqa: F541
            import shutil
            ...
            try:
                ...
                print(f"[PDB] === DESCARGA COMPLETADA ===")  # noqa: F541
                return final_file_path
    ...
    def _validate_pdb_content(self, pdb_content):
        ...
        try:
            ...
            has_header = any(
                line.startswith("HEADER") for line in lines[:20]
            )  # Buscar en más líneas
            has_end = any(
                line.startswith("END") for line in lines[-20:]
            )  # Buscar en más líneas

            print(
                f"Validación PDB: ATOMS={has_atoms}, HEADER={has_header}, END={has_end}"
            )
            print(
                f"Total líneas: {len(lines)}, Primeras líneas: {[l[:10] for l in lines[:3] if l.strip()]}"
            )
    ...
    def _clean_pdb_content(self, pdb_content, pdb_id):
        ...
        try:
            ...
            result = "\n".join(cleaned_lines)
            print(
                f"PDB procesado: {len(lines)} -> {len(cleaned_lines)} líneas, {atom_count} átomos válidos"
            )
            return result
    ...
    def _fix_valence_issues(self, mol, compound_name):
        ...
        try:
            ...
            try:
                ...
                Chem.SanitizeMol(
                    mol_copy, sanitizeOps=Chem.SANITIZE_ALL ^ Chem.SANITIZE_PROPERTIES
                )
    ...
    def create_sdf_file(self, drug_name):
        ...
        if os.path.exists(file_path):
            ...
            if os.path.getsize(file_path) > 0:
                ...
                print(
                    f"Archivo SDF {drug_name}.sdf existe pero está vacío, re-generando..."
                )
        ...
        try:
            ...
            mol = Chem.AddHs(mol)
            print(
                f"Hidrógenos explícitos añadidos. Átomos totales: {mol.GetNumAtoms()}"
            )
            ...
            try:
                ...
                if embed_result == -1:
                    print(
                        "Advertencia: Falló embed inicial, intentando método alternativo..."
                    )
                ...
                if embed_result != -1:
                    ...
                    try:
                        ...
                        print(
                            "Advertencia: Optimización UFF falló, usando geometría sin optimizar"
                        )
                else:
                    print("Error: No se pudieron generar coordenadas 3D")
                    error_msg = f"No se pudo generar la estructura 3D para {drug_name}. El medicamento puede tener una estructura molecular compleja."  # noqa: E501
                    return {
                        "error_type": "sdf_generation_failed",
                        "error": error_msg,
                        "user_message": _(
                            "No se pudo generar la estructura 3D del medicamento. Esto puede ocurrir con moléculas muy complejas o con metales de coordinación como Cisplatin."  # noqa: E501
                        ),
                    }
                ...
                error_msg = (
                    f"Error generando estructura 3D para {drug_name}: {str(geom_error)}"
                )
                return {
                    "error_type": "sdf_generation_error",
                    "error": error_msg,
                    "user_message": _(
                        "Error al generar la estructura 3D del medicamento. Algunos compuestos (como los que contienen metales) pueden requerir preparación manual."  # noqa: E501
                    ),
                }
    ...
    def build_docking_result_response(
        self,
        result: dict,
        drug: str,
        gene: str,
        ...
        if result.get("binding_affinity") and not result.get("error"):
            try:
                print(
                    f"[Visualizer] Preparando archivos para 3D viewer..."  # noqa: F541
                )  # noqa: F541
                ...
                if visualizer_result.get("success"):
                    print(
                        f"[Visualizer] ✓ Archivos preparados para visualización: {visualizer_result['experiment_key']}"
                    )
                    ...
                    print(
                        f"[Visualizer] ⚠ Error preparando visualización: {visualizer_result.get('error', 'Error desconocido')}"  # noqa: E501
                    )

            except Exception as visualizer_error:
                print(
                    f"[Visualizer] Error actualizando carpeta 3D_viewer: {visualizer_error}"
                )
                import traceback
        ...
        if result.get("error") or result.get("success") == False:  # noqa: E712
            response = {
                "type": "docking_error",
    ...
    def run_autodock_vina(
        self, receptor_path, drug_path, custom_config=None, user_id=None
    ):
        """
        ...
        try:
            ...
            print(f"[DEBUG] Inicializando DockerVinaService:")  # noqa: F541
            ...
            result = vina_service.run_vina_docking(
                receptor_path=receptor_path, drug_path=drug_path, config=default_config
            )
            ...
            if result["success"]:
                ...
                formatted_result = {
                    ...
                    "visualization_data": {
                        "pdb_content": self._get_output_pdb_content(
                            result.get("output_files", [])
                        ),
                        "binding_site": self._calculate_binding_site_from_results(
                            result
                        ),
                    },
                }
    ...
    def prepare_combined_database_context(self, genes_df, drugs_df, user_query):
        ...
        if not drugs_df.empty:
            ...
            context += (
                f"Columnas de medicamentos: {', '.join(drugs_df.columns.tolist())}\n"
            )
            ...
            for col in ["Name", "Description", "Indications"]:
                if col in drugs_df.columns:
                    ...
                    if not matches.empty:
                        ...
                        context += matches.head(3)[["Name", "Description"]].to_string(
                            index=False
                        )
                        break
        ...
        try:
            ...
            if drug_matches.empty:
                ...
                if not drugs_df.empty and "Name" in drugs_df.columns:
                    similar_drugs = self.find_similar_entries(
                        drugs_df, drug_name, "Name"
                    )
                    validation_result["error"] = _(
                        "Drug '{drug_name}' not found in database."
                    ).format(drug_name=drug_name)
                    if similar_drugs:
                        validation_result["suggestions"].extend(
                            [
                                _("Similar drugs available: {drugs}").format(
                                    drugs=", ".join(similar_drugs[:5])
                                )
                            ]
                        )
            ...
            if gene_matches.empty:
                ...
                if not genes_df.empty:
                    similar_symbols = (
                        self.find_similar_entries(genes_df, gene_name, "hgnc_symbol")
                        if "hgnc_symbol" in genes_df.columns
                        else []
                    )
                    similar_genes = (
                        self.find_similar_entries(genes_df, gene_name, "gene_name")
                        if "gene_name" in genes_df.columns
                        else []
                    )

                    validation_result["error"] = _(
                        "Gene '{gene_name}' not found in database."
                    ).format(gene_name=gene_name)
                    suggestions = []
                    if similar_symbols:
                        suggestions.append(
                            _("Similar symbols: {symbols}").format(
                                symbols=", ".join(similar_symbols[:5])
                            )
                        )
                    if similar_genes:
                        suggestions.append(
                            _("Similar genes: {genes}").format(
                                genes=", ".join(similar_genes[:5])
                            )
                        )
                    ...
                    if suggestions:
                        ...
                        available_genes = (
                            genes_df["hgnc_symbol"].dropna().head(10).tolist()
                            if "hgnc_symbol" in genes_df.columns
                            else []
                        )
                        validation_result["suggestions"] = [
                            _("Available genes: {genes}").format(
            ...
            if pd.isna(pdb_structures) or not str(pdb_structures).strip():
                validation_result["error"] = _(
                    "Gene '{gene_name}' has no PDB structures available."
                ).format(gene_name=gene_name)
                # Buscar genes con estructuras PDB (genes_df ya está disponible)
                if not genes_df.empty and "pdb" in genes_df.columns:
                    genes_with_pdb = genes_df[
                        genes_df["pdb"].notna() & (genes_df["pdb"].str.strip() != "")
                    ]
                    if not genes_with_pdb.empty and "hgnc_symbol" in genes_df.columns:
                        available_with_pdb = (
                            genes_with_pdb["hgnc_symbol"].head(5).tolist()
                        )
                        validation_result["suggestions"] = [
            ...
            validation_result["info"] = {
                "drug_info": {
                    ...
                    "molecular_formula": drug_data.get(
                        "Molecular_Formula"
                    ),  # Nueva columna
                    "molecular_weight": drug_data.get(
                        "Molecular_Weight"
                    ),  # Nueva columna
                    "smiles": drug_data.get("SMILES"),  # Columna 'SMILES' con mayúscula
                    "description": drug_data.get(
                        "Description"
                ...
                "gene_info": {
                    ...
                    "disease": gene_data.get("disease_info"),
                    "pdb_count": len(
                        [p.strip() for p in str(pdb_structures).split(";") if p.strip()]
                    ),
                },
            }
            ...
            return {
                ...
                "suggestions": [
                    "Verifica que el archivo Excel esté disponible y tenga el formato correcto"
                ],
    ...
    def find_drug_matches(self, drug_name):
        ...
        exact_matches = drugs_df[
            drugs_df[DRUG_COLUMNS["NAME"]].str.lower() == drug_name.lower()
        ]
        ...
        partial_matches = drugs_df[
            drugs_df[DRUG_COLUMNS["NAME"]].str.contains(drug_name, case=False, na=False)
        ]
    ...
    def find_gene_matches(self, gene_name):
        ...
        for col in available_columns:
            ...
            # Búsqueda parcial
            partial_matches = genes_df[
                genes_df[col].str.contains(gene_name, case=False, na=False)
            ]
    ...
    def generate_experiment_analysis(self, drug_name, gene_name, pdb_id):
        ...
        try:
            ...
            return {
                ...
                "drug_molecular_formula": clean_value(
                    drug_info.get("molecular_formula")
                ),
    ...
    def _preprocess_pdb_with_obabel_cli(self, pdb_file_path, pdb_id):
        ...
        try:
            ...
            if not obabel_path:
                print(
                    f"[obabel] Error: No se encontró el ejecutable obabel"  # noqa: F541
                )  # noqa: F541
                return None
            ...
            obabel_command = [
                ...
                "-O",
                temp_path,  # Archivo de salida temporal
                "-p",
                "7.4",  # pH fisiológico
                "-h",  # Añadir hidrógenos
            ]

            print(f"[obabel] === EJECUTANDO COMANDO OBABEL ===")  # noqa: F541
            ...
            print(
                f"[obabel] Tamaño archivo entrada: {os.path.getsize(pdb_file_path)} bytes"
            )

            # Ejecutar comando obabel con logs detallados
            print(f"[obabel] Iniciando subproceso...")  # noqa: F541
            ...
            result = subprocess.run(
                ...
                cwd=os.path.dirname(
                    obabel_path
                ),  # Ejecutar desde el directorio de obabel
            )
            ...
            if result.stdout:
                print(f"[obabel] === STDOUT ===")  # noqa: F541
                for line in result.stdout.strip().split("\n"):
                    if line.strip():
                        print(f"[obabel] STDOUT: {line}")
                print(f"[obabel] === FIN STDOUT ===")  # noqa: F541

            if result.stderr:
                print(f"[obabel] === STDERR ===")  # noqa: F541
                for line in result.stderr.strip().split("\n"):
                    if line.strip():
                        print(f"[obabel] STDERR: {line}")
                print(f"[obabel] === FIN STDERR ===")  # noqa: F541

            if result.returncode == 0:
                # Validar que el archivo temporal no esté vacío
                if not os.path.exists(temp_path) or os.path.getsize(temp_path) == 0:
                    print(
                        f"[obabel] Error: Archivo procesado está vacío o no existe"  # noqa: F541
                    )  # noqa: F541
                    if result.stderr:
                ...
                print(
                    f"[obabel] Preprocesamiento completado exitosamente"  # noqa: F541
                )  # noqa: F541
                ...
                # Error en el comando
                print(
                    f"[obabel] Error en preprocesamiento (código: {result.returncode})"
                )
                if result.stderr:
            ...
            print(
                f"[obabel] Error: Comando 'obabel' no encontrado. Instala OpenBabel CLI"  # noqa: F541
            )
            return None
        except Exception as e:
    ...
    def _find_obabel_executable(self):
        ...
        try:
            ...
            print(
                "[obabel] No se encontró el ejecutable obabel en las ubicaciones esperadas"
            )
            return None
    ...
    def _apply_secondary_pdb_fixes(self, pdb_file_path, pdb_id):
        ...
        try:
            ...
            if total_corrections > 0:
                print(f"[PDBFix] Correcciones aplicadas:")  # noqa: F541
                if corrections_applied["waters_removed"] > 0:
                    print(
                        f"  - Moléculas de agua eliminadas: {corrections_applied['waters_removed']}"
                    )
                if corrections_applied["heterogens_removed"] > 0:
                    print(
                        f"  - Heteroátomos no proteicos eliminados: {corrections_applied['heterogens_removed']}"
                    )
                if corrections_applied["invalid_atoms_fixed"] > 0:
                    print(
                        f"  - Átomos inválidos corregidos: {corrections_applied['invalid_atoms_fixed']}"
                    )
                if corrections_applied["duplicate_atoms_removed"] > 0:
                    print(
                        f"  - Átomos duplicados eliminados: {corrections_applied['duplicate_atoms_removed']}"
                    )
                print(f"[PDBFix] Total: {total_corrections} correcciones aplicadas")
                print(f"[PDBFix] Líneas finales: {len(corrected_lines)}")
            else:
                print(
                    f"[PDBFix] No se requirieron correcciones adicionales"  # noqa: F541
                )  # noqa: F541
        ...
        try:
            ...
            if line.startswith(("ATOM", "HETATM")):
                ...
                try:
                    ...
                    line = (
                        line[:30] + f"{x:8.3f}" + f"{y:8.3f}" + f"{z:8.3f}" + line[54:]
                    )
    ...
    def _renumber_pdb_residues(self, pdb_file_path, pdb_id):
        ...
        try:
            ...
            print(
                f"[PDBRenumber] ✓ Renumeración completada: {len(residue_map)} residuos únicos"
            )
