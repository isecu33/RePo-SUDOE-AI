...
def protein_lookup_api(request):
    ...
    try:
        ...
        matches = ProteinDatabase.objects.filter(
            hgnc_symbol__icontains=query
        ) | ProteinDatabase.objects.filter(
            gene_name__icontains=query
        ) | ProteinDatabase.objects.filter(
            synonyms__icontains=query
        )
...
def structure_info_api(request):
    ...
    try:
        ...
        if not pdb_code:
            return JsonResponse({
                'success': False,
                'error': 'pdb_code parameter is required'
            }, status=400)
        
        # Find proteins with this PDB code
        proteins = ProteinDatabase.objects.filter(
            pdb_codes__icontains=pdb_code
        )
