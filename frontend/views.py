...
def chat_message(request):
    ...
    try:
        ...
        logger.info(f"Parámetros extraídos - Message: {message}, Session: {session_id}, "
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'error': f'Error importing QueryHandler: {str(e)}'
            }, status=500)
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'error': f'Error importing models: {str(e)}'
            }, status=500)
        
        # Activate language based on request body (priority) or cookie
        try:
            language_code = data.get('language') or request.COOKIES.get('django_language', 'es')
        ...
        try:
            if session_id:
                try:
                    ...
                    logger.debug(f"Chat session no encontrada, creada nueva: {chat_session.id}")
            else:
                chat_session = ChatSession.objects.create(user=request.user)
            ...
            return JsonResponse({
                'success': False,
                'error': f'Error handling chat session: {str(e)}'
            }, status=500)
        
        # Process query using the new simplified handler
        try:
            logger.info("Procesando query con QueryHandler...")
            logger.debug(f"Parámetros para process_query: user_query={message}, "
        ...
        if result_type in ['api_error', 'processing_error', 'sdf_generation_error', 'file_preparation_error']:
            ...
            response_data['message'] = result['error']  # Usar 'error' como 'message' para el chat
            ...
            response_data['experiment_analysis'] = result.get('experiment_analysis', {})  # NUEVO
            ...
            response_data['message'] = result.get('message', str(_('Docking experiment completed successfully')))
            ...
            response_data['success'] = False
            response_data['error'] = result.get('error', str(_('Error during docking experiment')))
            response_data['message'] = result.get('error', str(_('Error during docking experiment')))
        ...
        logger.error(f"=== ERROR CRÍTICO EN CHAT_MESSAGE ===")
...
def upload_file(request):
    ...
    try:
        ...
        if file_extension not in allowed_extensions.get(file_type, []):
            logger.error(f"Tipo de archivo no válido: {file_extension} para {file_type}")
            return JsonResponse({
                'success': False,
                'error': f'Invalid file type for {file_type}. Allowed: {", ".join(allowed_extensions.get(file_type, []))}'
            }, status=400)
        ...
        if file_extension in ['.pdb', '.ent', '.sdf', '.mol', '.mol2']:
            try:
                ...
                molecular_result = MolecularUtils.process_uploaded_molecule(uploaded_file)
...
def run_docking(request):
    ...
    try:
        ...
        if not receptor_file or not drug_file:
            ...
            return JsonResponse({
                'success': False,
                'error': 'Receptor and drug files are required'
            }, status=400)
        ...
        if not os.path.exists(drug_file):
            ...
            return JsonResponse({
                'success': False,
                'error': f'Drug file not found: {drug_file}'
            }, status=400)
        ...
        logger.info(f"Running docking with files: receptor={receptor_file}, drug={drug_file}")
        ...
        logger.info(f"Docking completado exitosamente")
...
def get_chat_history(request):
    ...
    try:
        ...
        chat_sessions = ChatSession.objects.filter(user=request.user).order_by('-updated_at')[:7]
...
def serve_output_file(request, filename):
    ...
    try:
        ...
        for exp_key, exp_data in experiments.items():
            ...
            if exp_data.get('pdbqt_file') and os.path.basename(exp_data['pdbqt_file']) == filename:
                file_path = exp_data['pdbqt_file']
                logger.info(f"Found file in user's experiment: {exp_key}")
                break
            elif exp_data.get('log_file') and os.path.basename(exp_data['log_file']) == filename:
        ...
        if not file_path:
            ...
            if os.path.exists(potential_path):
                ...
                logger.info(f"Found file in user's output folder")
...
def list_user_experiments(request):
    ...
    try:
        ...
        logger.info(f"[list_user_experiments] Output dir exists: {os.path.exists(output_dir)}")

        experiments = []

        # Check if user output directory exists
        if not os.path.exists(output_dir):
            logger.warning(f"[list_user_experiments] Output directory does not exist: {output_dir}")
            return JsonResponse({
                'success': True,
                'experiments': []
            })
        ...
        for pdbqt_file in pdbqt_files:
            ...
            receptor_file_pdbqt = os.path.join(output_dir, f"{experiment_name.split('_')[0]}.pdbqt")
            receptor_file_pdb = os.path.join(output_dir, f"{experiment_name.split('_')[0]}.pdb")
            ...
            experiments.append({
                ...
                'files': {
                    'output': basename,
                    'log': os.path.basename(log_file) if os.path.exists(log_file) else None,
                    'receptor': os.path.basename(receptor_file) if receptor_file and os.path.exists(receptor_file) else None
                }
            })
...
def serve_input_file(request, filename):
    ...
    try:
        ...
        if user_input_path.exists():
            ...
            logger.info(f"Found file in user's input folder")
...
def download_experiment_zip(request, experiment_key):
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            receptor_path = os.path.join(query_handler.final_input_dir, receptor_filename)
            if not os.path.exists(receptor_path):
                receptor_path = os.path.join(query_handler.temp_input_dir, receptor_filename)
            ...
            if not os.path.exists(ligand_path):
                ligand_path = os.path.join(query_handler.temp_input_dir, ligand_filename)
        ...
        if files_added == 0:
            ...
            return JsonResponse({
                'success': False,
                'error': 'No experiment files found'
            }, status=404)

        # Prepare response with ZIP file
        zip_buffer.seek(0)
        response = HttpResponse(zip_buffer.getvalue(), content_type='application/zip')
        response['Content-Disposition'] = f'attachment; filename="{experiment_key}_experiment.zip"'

        logger.info(f"Successfully created ZIP with {files_added} files for experiment: {experiment_key}")
        return response
...
def get_visualizer_files(request):
    ...
    try:
        ...
        from core.services.visualizer_file_manager import get_file_manager, prepare_latest_experiment
        
        # Prepare the latest experiment files
        result = prepare_latest_experiment()
        
        if not result.get('success'):
            logger.warning(f"No experiment files prepared: {result.get('error', 'Unknown error')}")
            return JsonResponse({
                'success': False,
        ...
        if not file_contents:
            ...
            return JsonResponse({
                'success': False,
                'error': 'No file contents available'
            })
        ...
        logger.info(f"Successfully prepared visualizer files for experiment: {result['experiment_key']}")
        return JsonResponse(response_data)
        ...
        return JsonResponse({
            'success': False,
            'error': 'Visualizer file manager not available'
        }, status=500)
...
def get_available_experiments(request):
    ...
    try:
        ...
        logger.info(f"Found {len(experiments)} available experiments for user {user_id}")
...
def get_specific_experiment(request, experiment_key):
    ...
    try:
        ...
        if not result.get('success'):
            logger.warning(f"Failed to prepare experiment {experiment_key}: {result.get('error', 'Unknown error')}")
            return JsonResponse({
                'success': False,
                'error': result.get('error', 'Experiment not found')
            })
...
def get_pdb_center(request, pdb_id):
    ...
    try:
        ...
        if not os.path.exists(pdb_path):
            ...
            return JsonResponse({
                'success': False,
                'error': f'PDB {pdb_id} no encontrado en input/'
            }, status=404)
