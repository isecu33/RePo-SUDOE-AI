...
class CustomUserCreationForm(UserCreationForm):
    ...
    class Meta:
        ...
        fields = (
            ...
            'institution', 'department', 'position', 'research_area',
            'orcid_id', 'password1', 'password2'
        )
    ...
    def clean_orcid_id(self):
        ...
        if orcid_id:
            ...
            if not re.match(pattern, orcid_id):
                raise ValidationError("Please enter a valid ORCID ID format (0000-0000-0000-0000)")
        return orcid_id
...
class AccessRequestForm(forms.ModelForm):
    ...
    class Meta:
        model = AccessRequest
        fields = [
            'purpose', 'project_description', 'expected_usage', 
            'supervisor_email', 'institutional_verification'
        ]
        widgets = {
            ...
            'project_description': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 4,
                'placeholder': 'Please describe your project and how you plan to use RePo-SUDOE-AI. Include research objectives, methodology, and expected outcomes.'
            }),
            'expected_usage': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Describe your expected usage pattern (e.g., frequency of use, number of simulations, duration of project)'
            }),
            'supervisor_email': forms.EmailInput(attrs={
                'class': 'form-control',
                'placeholder': 'Supervisor or Principal Investigator email (if applicable)'
            }),
            'institutional_verification': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
...
class ProfileUpdateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'position', 'research_area', 'orcid_id'
        ]
        widgets = {
...
class AdminReviewForm(forms.Form):
    ...
    def clean(self):
        ...
        if action in ['reject', 'request_more_info'] and not admin_notes:
            raise ValidationError("Admin notes are required when rejecting or requesting more information.")
