...
def verify_email(request, token):
    ...
    try:
        ...
        messages.success(
            request,
            "Email address verified successfully! You can now log in once your account is approved.",
        )
        return redirect("accounts:login")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(
                    request, "No active account found with this email address."
                )
    else:
...
def profile(request):
    ...
    return render(
        ...
        {"user": user, "access_request": access_request},
    )
...
def admin_dashboard(request):
    ...
    pending_requests = AccessRequest.objects.filter(status="pending").order_by(
        "-created_at"
    )
    recent_requests = AccessRequest.objects.exclude(status="pending").order_by(
        "-updated_at"
    )[:10]
...
def review_access_request(request, request_id):
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            messages.success(
                request,
                f"Access request approved for {access_request.user.get_full_name()}",
            )
            ...
            messages.success(
                request,
                f"Access request rejected for {access_request.user.get_full_name()}",
            )
    ...
    return render(
        request, "accounts/review_request.html", {"access_request": access_request}
    )
...
def send_password_reset_email(user, request):
    ...
    reset = PasswordReset.objects.create(user=user, ip_address=get_client_ip(request))
...
def notify_admin_new_registration(access_request):
    ...
    html_message = render_to_string(
        "accounts/emails/simple_admin_notification.html",
        {
...
def send_rejection_email(user, reason):
    ...
    html_message = render_to_string(
        "accounts/emails/account_rejected.html",
        {
            "user": user,
            "reason": reason,
        },
...
def send_more_info_email(user, message):
    ...
    html_message = render_to_string(
        "accounts/emails/more_info_request.html",
        {
            "user": user,
            "message": message,
        },
